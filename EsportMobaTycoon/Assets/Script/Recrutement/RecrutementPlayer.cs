...
public class RecrutementPlayer : MonoBehaviour
{
    ...
    public void AddToTeam()
    {
        ...
        {
            Debug.Log("Ce joueur est d�j� dans l'�quipe.");
            return;
        }

        if (i_manager.GetPlayerByRole(i_selected.i_role) != null)
        {
            Debug.Log("Ce r�le est d�j� pris.");
            return;
        }

        if (i_manager.TeamPlayers.Count >= 5)
        {
            Debug.Log("L'�quipe est compl�te.");
            return;
        }

        selectedTeam.Add(selected);
        Debug.Log($"{selected.i_name} ajout�E�El'�quipe en tant que {GetRoleName((int)(selected.i_currentRole))}.");

        // Cr�e une UI dans le bon panel
    ...
    public void MovePlayerToNewRole(int i_newRole)
    {
        ...
        if (i_manager.TeamPlayers.Contains(i_selected))
        {
            ...
            Debug.Log($"{i_selected.i_name} d�plac� vers le r�le {GetRoleName(i_newRole)}");
    ...
    public void RemovePlayer(Player i_player)
    {
        ...
        Debug.Log($"{i_player.i_name} a �t� retir� de l'�quipe.");
